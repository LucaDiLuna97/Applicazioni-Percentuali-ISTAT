...
update;
					...
					array_push($arrayColonneDaBloc, $IdProprietario."_".$meseCiclato."_".$annoCiclato);
					...
					for($i = 1; $i <= 11; $i++){
						...
						if($nextAnno == false){
							$meseDaBloccare = $meseCiclato+$addMese;
							$annoDaBloccare = $annoCiclato;
							...
							$meseDaBloccare = 1+$addMese; //$addMese;
						...
						if($meseDaBloccare > 12){
							
							//se meseDaBloccare è maggiore di 12
							
							$meseDaBloccare = 1;  //meseDaBloccare è 1 quindi Gennaio
							$annoDaBloccare++;    //annoDaBloccare aumenta di 1
						...
						$sqlAnnoSuccessivo = <<<sql
									...
									AND	   IdCommessa = $IdCommessa
									AND    Mese = $meseDaBloccare
									AND	   Anno = $annoDaBloccare
sql;
...
update;
							...
							$insertRecord = <<<insert
									...
									VALUES ($IdProprietario, $IdCommessa, NULL, $meseDaBloccare, $annoDaBloccare, 1)
insert;
							ntxQuery($insertRecord);
							//jhp_log($insertRecord);
						}
						
						if($meseDaBloccare < 10){
							
							$meseDaBloccare = "0".$meseDaBloccare;
						}
						array_push($arrayColonneDaBloc, $IdProprietario."_".$meseDaBloccare."_".$annoDaBloccare);
					}	
